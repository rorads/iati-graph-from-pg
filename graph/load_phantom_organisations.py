# graph/load_phantom_organisations.py
...
SOURCE_TABLE = "phantom_organisations"
NEO4J_NODE_LABEL = "PhantomOrganisation"
NEO4J_ID_PROPERTY = "reference"  # Changed from organisationidentifier to reference based on SQL model
NEO4J_NAME_PROPERTY = "name" # Explicit name for the organisation name

# Columns to load from PostgreSQL - updated based on the SQL model
SOURCE_COLUMNS = [
    "reference",                        # Unique identifier (ref from various tables)
    "distinct_narratives",              # Array of distinct narrative texts
    "phantom_in_participatingorg",      # Boolean flag 
    "phantom_in_transaction_provider",  # Boolean flag
    "phantom_in_transaction_receiver",  # Boolean flag 
    "phantom_in_orgbudget_recipient"    # Boolean flag
]
...
def load_phantom_organisation_nodes(pg_conn, neo4j_driver, batch_size):
    """Loads PhantomOrganisation nodes from PostgreSQL to Neo4j, including all specified columns."""
    ...
    pg_cursor = pg_conn.cursor(name='fetch_phantom_orgs', cursor_factory=psycopg2.extras.DictCursor)
    ...
    for col in SOURCE_COLUMNS:
        ...
        # and also skip name which will be derived from distinct_narratives
        if col != NEO4J_ID_PROPERTY:
            # Use row[col] for accessing data in the batch map
            set_clauses.append(f"n.{prop_name} = row.{prop_name}")
    
    # Add an explicit SET for the name property derived from distinct_narratives
    set_clauses.append(f"n.{NEO4J_NAME_PROPERTY} = CASE WHEN row.{NEO4J_NAME_PROPERTY} IS NOT NULL THEN row.{NEO4J_NAME_PROPERTY} ELSE null END")
         ...
         while True:
            ...
            for row_dict in [dict(row) for row in batch_data]:
                ...
                for col in SOURCE_COLUMNS:
                    ...
                    if col == "distinct_narratives" and value:
                        # Extract the first non-empty narrative (if any) to use as the name
                        if isinstance(value, list) and len(value) > 0 and value[0]:
                            sanitised_item[NEO4J_NAME_PROPERTY] = value[0]  # Use first narrative as name
                        # Keep the full array as a separate property
                        prop_name = col.replace("-", "_")
...
def main():
    ...
    try:
        print("--- Starting Phantom Organisation Load ---")
        neo4j_driver = get_neo4j_driver()
        pg_conn = get_postgres_connection()

        success = load_phantom_organisation_nodes(pg_conn, neo4j_driver, batch_size)
        ...
        if success:
            print("\nPhantom Organisation loading process finished successfully.")
        else:
            print("\nPhantom Organisation loading process finished with errors or was interrupted.", file=sys.stderr)
            sys.exit(1)
